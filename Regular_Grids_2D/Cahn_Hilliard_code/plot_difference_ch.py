...
def plot_single_diff(mid_outputs, save_path, layer, sample_idx, step_idx, prefix='inference'):
    """绘制单个样本的 Initial Condition、Ground Truth、Prediction 和 Absolute Error，并分别保存图像。
    - Colorbar 置于底部；
    - 使用 PRL 推荐字体（如 Times New Roman）；
    - 去除坐标轴 grid；
    """
...
def plot_slice(mid_outputs, save_path, layer, sample_idx, step_idx, x_coords=[0, 50, 100], prefix='inference'):
    """绘制中间输出在固定 x 坐标处的 y 切片图，分别画 Initial, Prediction, Ground Truth"""
    ...
    def plot_single_slice(data_tensor, title, filename):
        ...
        plt.legend(loc='upper right', fontsize=font_size - 4)  # 图例放在右上角
