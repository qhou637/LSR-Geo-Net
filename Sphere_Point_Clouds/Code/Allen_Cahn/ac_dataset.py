...
def train_val_dataset(dataset, seed=42, test_split=0.2, val_split=0.2):
    ...
    # 用临时训练集保存 train + val
    train_item = Subset(dataset, train_idx)
    
    # 第二次划分：从 train_item 中划分出 val
    train_sub_idx, val_sub_idx = train_test_split(
        list(range(len(train_item))), test_size=val_split, random_state=seed, shuffle=True
    )
    
    # 构建实际子集
    datasets = {
        'train': Subset(train_item, train_sub_idx),
        'val': Subset(train_item, val_sub_idx),
